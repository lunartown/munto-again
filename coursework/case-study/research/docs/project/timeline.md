...
| 6.5. Current-State Audit | 문토 현재 플로우·화면·정보 구조 분석 | 진행 중 | 화면 39장·탐색/참여 플로우 1차·근거 매핑, 미확인 상태 보강 |
...
1. 참석·체크인·후기 화면과 챌린지 전용 승인·취소 상태를 추가 확인하거나 미확인으로 고정한다.
2. 현재 화면·정보 인벤토리의 근거 ID와 실제 캡처를 한 번 더 대조한다.
3. Sprint 1.5에서 우선 문제 1개와 `클럽 탐색 → 활동 적합성·지속성 판단 → 가입 결정` 범위를 정리한다.
4. 아이디에이션에서 활동 목적·반복 방식·최근 활동·활성 멤버·운영 규칙을 보여주는 대안을 발산하고, 하나를 Sprint 2 와이어프레임으로 구체화한다.
