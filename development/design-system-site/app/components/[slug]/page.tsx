...
const docs = {
  "participated-gathering-card": {
    name: "Participated Gathering Card", category: "CONTENT COMPONENT", description: "후기와 참여한 정기모임 정보를 연결합니다. 공개 여부와 노출 맥락에 따라 카드·미니멀·목록 형태로 전환됩니다.",
    demo: <GatheringVariantDemo />,
...
export default async function ComponentDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <DocsLayout toc={[{ href: "#overview", label: "Overview" },{ href: "#anatomy", label: "Anatomy" },{ href: "#variants", label: "Variants & states" },{ href: "#usage", label: "Usage" },{ href: "#gaps", label: "보강 필요" }]}>
      <header className="component-hero" id="overview"><p className="eyebrow">{doc.category}</p><h1>{doc.name}</h1><p>{doc.description}</p></header>
      ...
      <section className="doc-section" id="variants"><div className="doc-section-head"><div><h2>Variants & states</h2><p>속성과 상태를 조합해 콘텐츠의 노출 방식과 상호작용을 구분합니다.</p></div></div><div className="variant-list">{doc.variants.map((variant) => <code key={variant}>{variant}</code>)}</div></section>
      <section className="doc-section" id="usage"><div className="doc-section-head"><div><h2>Usage</h2><p>컴포넌트의 역할과 화면 맥락에 맞춰 아래 원칙을 적용합니다.</p></div></div><ol className="usage-rules">{doc.usage.map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span><p>{item}</p></li>)}</ol></section>
      <section className="doc-section gap-section" id="gaps"><div className="doc-section-head"><div><h2>보강이 필요한 부분</h2><p>웹과 다양한 콘텐츠 조건에서 일관되게 사용하려면 아래 기준을 추가해야 합니다.</p></div><StatusBadge kind="pending">정의 필요</StatusBadge></div><ul className="gap-list compact">{doc.gaps.map((item) => <li key={item}><span>{item}</span></li>)}</ul></section>
    </DocsLayout>
  );
}
