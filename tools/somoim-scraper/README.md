...
펨코는 Python 어댑터에 포함하지 않는다. URL 발견 후 Chrome에서 원문을 직접 확인해 `data/fmkorea-full-posts.jsonl`로 저장했다.
...
- 원문: `data/records.jsonl` 439건
- 사이트별: 네이트판 303 / 더쿠 53 / 펨코 47 / 개드립 20 / 디시인사이드 16
- 펨코: 후보 URL 49개 중 원문 47개 확보, 삭제 글 2개 미확보
- 검증: JSON 파싱 오류 0건, URL 중복 0건, 빈 제목 0건
- 사용한 검색어: 소모임 어플·앱·후기·정모·가입·탈퇴·모임장·동호회·활동·운영·추천·다녀온

사이트별 검색 방식이 다르고 확률 표본이 아니므로, 전체 439건을 합쳐 불만 비율이나 사용자 비율을 계산하지 않는다. 소모임 앱 관련 경험담을 선별하기 위한 원문 후보군으로 사용한다.

## 한계 / 다음
- 439건은 아직 소모임 앱 관련성 전수 검토 전인 원문 후보군임.
- 네이트판 본문은 머리말/꼬리말 제거가 휴리스틱이라 일부 지저분할 수 있음.
- 펨코 삭제 글 2개는 `data/fmkorea-unavailable.jsonl`에 URL과 실패 사유만 기록함.
