...
- `group_members`: 공개 상세에 노출된 멤버 중 최대 12명의 표시 이름·프로필 이미지 URL
- `group_articles`: 공개 게시판의 최신 글 최대 12건과 분류·본문·반응 수
- `group_photos`: 공개 사진첩의 최신 사진 최대 12건과 반응 수

상세 수집은 명시한 `gid`만 요청하며, 화면 구성에 필요한 공개 필드만 선별해 적재합니다.
멤버는 표시 이름·프로필 이미지·모임장 여부만 저장하고 계정 상태나 활동 정보는 저장하지 않습니다.
