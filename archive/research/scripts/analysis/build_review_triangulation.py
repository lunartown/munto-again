...
CONDITION_META = {
    ...
    "D": {
        ...
        "support": "strong",
        "interpretation": (
            "취소·환불·문의 해결과 신고·제재·신원·안전 문제가 두 플랫폼에서 "
            "직접 반복됨. 앱 기능 오류는 별도 군집으로 분리"
        ),
    },
    "E": {
        "name": "반복 참여·작은 기여",
...
def main() -> None:
    ...
    assigned_ids = [
        card_id
        for ids in cluster_card_ids.values()
        for card_id in ids
    ]
    unknown_ids = sorted(set(assigned_ids) - set(cards_by_id))
    missing_ids = sorted(set(cards_by_id) - set(assigned_ids))
    duplicate_ids = sorted(
        card_id
        for card_id, count in Counter(assigned_ids).items()
        if count > 1
    )
    if unknown_ids or missing_ids or duplicate_ids:
        raise ValueError(
            "Affinity clusters must form a complete, non-overlapping card partition: "
            f"unknown={unknown_ids}, missing={missing_ids}, duplicates={duplicate_ids}"
        )
